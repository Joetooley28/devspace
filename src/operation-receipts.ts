...
export class OperationReceiptManager {
  ...
  async run<T>(input: RunRecoverableOperationInput<T>): Promise<RecoverableOperationResult<T>> {
    ...
    // Retry safety comes first: a known live receipt must remain replayable even
    // if unrelated expired receipts cannot currently be compacted.
    ...
    if (tombstone) {
      ...
      throw new Error(
        `Operation ${input.operationId} was already executed, but its stored result has expired. Do not execute it again; inspect current state and use a new operation_id for any new action.`,
      );
    }

    this.compactExpiredReceipts();

    // Compaction can move this key to a tombstone only when another invocation
    // races through shared state between the checks above and here.
